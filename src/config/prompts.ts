    ...
    storyboardImage: [
        {
            ...
            prompt: '【任务：请按照用户要求，多图生成9宫格动漫分镜设计稿】1.布局结构：9宫格，3行×3列。画面整体为标准分镜稿格式，每个宫格独立为一个镜头，宫格之间有清晰的白色分隔线。2.右上角预留镜头顺序编号区域（格式：No.1/No.2…）。3.所有分镜稿的宫格布局、风格、分辨率、标注区域格式完全统一，支持后续自动化拼接为长漫剧分镜序列。4.优化指令：优先保证分镜的功能性，其次提升画面的美观度，弱化非必要的背景细节，突出主体和镜头信息。5.镜头连贯，运镜丝滑。6.无文字。7.分析角色设定，多个宫格保证角色设定一致。8.用户要求:',
        },
        {
            id: 'image-none',
            label: '无模板',
