...
videosRouter.post('/storyboard-to-video', async (req: Request, res: Response, next: NextFunction) => {
  const startTime = Date.now();
  try {
    const { prompt, model, aspect_ratio, duration, private: isPrivate, reference_images } = storyboardToVideoSchema.parse(req.body);

    console.log('\n========== 分镜生成视频请求 ==========');
    console.log('请求参数:', JSON.stringify({ prompt, model, aspect_ratio, duration, private: isPrivate, reference_images }, null, 2));
